...
SemiDenseTracking::SemiDenseTracking()
{
   ...
   boost::filesystem::remove_all((ros::package::getPath("unkei")+"/src/map_and_poses").c_str());
   boost::filesystem::create_directory((ros::package::getPath("unkei")+"/src/map_and_poses").c_str());
   boost::filesystem::remove_all((ros::package::getPath("unkei")+"/src/evaluation").c_str());
   boost::filesystem::create_directory((ros::package::getPath("unkei")+"/src/evaluation").c_str());
   boost::filesystem::remove_all((ros::package::getPath("unkei")+"/src/results_depth_maps").c_str());
   boost::filesystem::create_directory((ros::package::getPath("unkei")+"/src/results_depth_maps").c_str());
    ...
    if  (*semidense_tracker->cont_frames > semidense_tracker->last_cont_frames)
    {
            ...
            if (semidense_mapper->do_initialization > 0.5)
            {
             ...
             /*if ( semidense_mapper->num_keyframes %2==0)
             {
                 ...
                 //ros::package::getPath("unkei")+"/src/results_depth_maps").c_str()
                // sprintf(buffer_evaluation,"src/dpptam/src/evaluation/evaluation%d.txt",semidense_mapper->num_keyframes);
                 sprintf (buffer_evaluation,(ros::package::getPath("unkei")+"/src/evaluation/evaluation%d.txt").c_str(),semidense_mapper->num_keyframes);
                     ...
                     ros::Publisher *odom_pub,ros::Publisher *pub_poses,ros::Publisher *vis_pub,cv::Mat &points3D_cam_to_print)
{
   ...
   char buffer[150];
    ...
    {
        sprintf (buffer,(ros::package::getPath("unkei")+"/src/map_and_poses/tracking.ply").c_str());
