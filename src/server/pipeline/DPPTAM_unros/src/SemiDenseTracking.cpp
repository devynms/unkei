    ...
    if  (*semidense_tracker->cont_frames > semidense_tracker->last_cont_frames)
    {
            ...
            if (semidense_mapper->do_initialization > 0.5)
            {
             ...
             optimize_camera(semidense_mapper->num_keyframes,semidense_tracker,semidense_mapper,*images,semidense_tracker->image_to_track,\
                            ...
                            semidense_tracker->image_gray, stamps_aux, time_stamps,semidense_mapper->mean_value, points3D_cam_to_print);
             ...
             /*if ( semidense_mapper->num_keyframes %2==0)
             {
                 ...
                 //getcwd()+"/src/results_depth_maps").c_str()
                // sprintf(buffer_evaluation,"src/dpptam/src/evaluation/evaluation%d.txt",semidense_mapper->num_keyframes);
                 sprintf (buffer_evaluation,(getcwd()+"/src/evaluation/evaluation%d.txt").c_str(),semidense_mapper->num_keyframes);
...
/*void     publish_camera_frame(cv::Mat R,cv::Mat t,ros::Publisher *vis_pub)
{
   ...
   marker.points.clear();
    ...
    marker.header.stamp = clock_t::now();
...
void optimize_camera(int num_keyframes,SemiDenseTracking *semidense_tracker,SemiDenseMapping *semidense_mapper,Imagenes &images,cv::Mat &image_to_track,\
                    ...
                    vector<double> &variances,cv::Mat &image_gray, double stamps, clock_t time_stamps,float mean_depth_value,\
                     cv::Mat &points3D_cam_to_print)
{
    for (int j = 0 ; j < pyramid_levels; j++)
    {
    ...
    //KEYWORD: cutting this out because we think its just for rviz
    ...
    //mTfBr.sendTransform(tf::StampedTransform(tfTcw,clock_t::now(), "dpptam/map", "dpptam/visualization_marker"));
...
void show_error1( cv::Mat points3D_cam, cv::Mat image,  cv::Mat image_print, int num_pixels_sd2project,cv::Mat &weight)
{
